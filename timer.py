...
class Application(tk.Frame):
 def __init__(self, M, *args, **kwargs):
  tk.Frame.__init__(self, M, *args, **kwargs)
  self.M = M
  self.running = False
  self.time = 0 
  self.hours = 0
 ...
 def build_interface(self):
  ...
  self.clock = tk.Label(self, text="00:00:00", font=("arial", 25), width=15)
  self.clock.grid(row=1, column=1, stick="S")
  self.time_label = tk.Label(self, text="hour min sec", font=("arial",12), width=15)
...
def timer(self): 
 ...
 def start(self):
  try:
   ...
   self.M.bind("<Return>", lambda x: self.stop())
   self.running = True
   self.timer()
 def stop(self):
 ...
 def pause(self):
   ...
   self.M.bind("<Return>", lambda x: self.resume())
 ...
 def resume(self):
  ...
  self.M.bind("<Return>", lambda x: self.pause())
